...
class text_widget(QWidget):
    ...
    def itUI(self):
        ...
        # global cstext
        global my_extension
        my_extension = QTextEdit()
        # my_extension.setText('py')
        ...
        hbox = QHBoxLayout()
        # if(my_extension.toPlainText()== 'py'):
        hbox.addWidget(text)
        # else:
...
class Widget(QWidget):
    ...
    def on_clicked(self, index):
        ...
        if nn[0]:
            # print(nn[0])
            my_extension.setText(nn[0][-2:])
            print(my_extension.toPlainText())
            f = open(nn[0],'r')
            with f:
                data = f.read()
                if(my_extension.toPlainText()=='py' or my_extension.toPlainText()=='cs'):
                    Python_Coloring.setmy_extension(my_extension.toPlainText())
                    text.setText(data)
                # else:
                #     cstext.setText(data)
                # self.initUI()
