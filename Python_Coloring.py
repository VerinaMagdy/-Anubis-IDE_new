...
class Highlighter(QSyntaxHighlighter):
    ...
    def __init__(self, document):
        ...
        python_rules += [(r'\b%s\b' % w, 0, STYLES['keyword'])
                  for w in Highlighter.python_keywords]
        python_rules += [(r'%s' % o, 0, STYLES['operator'])
                  for o in Highlighter.python_operators]
        python_rules += [(r'%s' % b, 0, STYLES['brace'])
                  for b in Highlighter.python_braces]
        ...
        self.python_rules = [(QRegExp(pat), index, fmt)
                      for (pat, index, fmt) in python_rules]
        

        cs_rules = []

        # Keyword, operator, and brace rules
        cs_rules += [(r'\b%s\b' % w, 0, STYLES2['keyword'])
                  for w in Highlighter.cs_keywords]
        cs_rules += [(r'%s' % o, 0, STYLES2['operator'])
                  for o in Highlighter.cs_operators]
        cs_rules += [(r'%s' % b, 0, STYLES['brace'])
                  for b in Highlighter.cs_braces]
        ...
        # Build a QRegExp for each pattern
        self.cs_rules = [(QRegExp(pat), index, fmt)
                      for (pat, index, fmt) in cs_rules]
    ...
    def highlightBlock(self, text):
        ...
        if(my_extension=='py'):
            for expression, nth, format in self.python_rules:
            ...
            for expression, nth, format in self.cs_rules:
                index = expression.indexIn(text, 0)
